...
const createCard = () => {
  ...
  const cardName = svg('text', 'FULL NAME', {
    transform: 'matrix(1 0 0 1 54.1064 428.1723)',
    id: 'svgname',
    class: 'st2 st5 st6',
  });

  const cardExpire = svg('text', '••/••', {
  ...
  const cardSecurity = svg('text', {
    transform: 'matrix(1 0 0 1 621.999 227.2734)',
    id: 'svgsecurity',
    class: 'st6 st7',
  });

  const nameBack = svg('text', {
  ...
  setChildren(cardСontainer, [creditCard]);
...
const createForm = () => {
  ...
  const inputExpirationDate = el('input#expirationdate', {
    type: 'text',
    inputmode: 'numeric',
  });
  const inputSecurityCode = el('input#securitycode', {
    type: 'text',
    inputmode: 'numeric',
  });
